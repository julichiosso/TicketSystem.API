    ...
    <main class="flex-1 p-8 space-y-6 overflow-y-auto h-screen custom-scrollbar">
            ...
            <div v-if="activeTab === 'dashboard' || activeTab === 'usuarios'" class="relative w-48 xl:w-64">
              <input v-if="activeTab === 'usuarios'" v-model="usersSearch" placeholder="Buscar usuarios..."
                ...
                :class="settingsStore.isDark
                  ...
                  : 'bg-white border-slate-200 placeholder-slate-400 focus:border-blue-400'" />
      ...
      <section v-else-if="activeTab === 'usuarios'" class="space-y-3">
        ...
        <div v-else-if="filteredUsers.length > 0">
          ...
          <div v-for="user in filteredUsers" :key="user.id"
            ...
            <div>
              ...
              <div class="text-xs" :class="settingsStore.isDark ? 'text-slate-500' : 'text-slate-400'">{{ user.email }}</div>
            </div>
            <div class="flex items-center gap-2">
        ...
        <div v-else class="flex flex-col items-center justify-center py-32 border-2 border-dashed rounded-[3rem]"
          :class="settingsStore.isDark ? 'border-slate-800' : 'border-slate-200'">
          ...
          <div class="rounded-2xl p-6 border transition-colors"
            ...
            <div class="space-y-3">
              <div v-for="kpi in kpis" :key="kpi.label"
                ...
                <span class="text-sm" :class="settingsStore.isDark ? 'text-slate-400' : 'text-slate-500'">{{ kpi.label }}</span>
          ...
          <div class="space-y-4">
            <div v-for="field in configFields" :key="field.label" class="space-y-2">
              <label class="text-xs font-bold uppercase tracking-widest"
                :class="settingsStore.isDark ? 'text-slate-500' : 'text-slate-400'">{{ field.label }}</label>
        ...
        <div class="rounded-2xl p-6 border transition-colors"
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <button @click="clearAuditLog"
              ...
              :class="settingsStore.isDark ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'">
              Limpiar Auditoría
            </button>
