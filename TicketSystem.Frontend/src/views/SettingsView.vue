    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="flex items-center gap-4 mb-12">
        <button @click="router.back()"
          ...
          :class="'bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 border border-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-300'">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <button @click="settingsStore.setThemeMode('light')"
              ...
              :class="settingsStore.themeMode === 'light'
                ? 'border-blue-500'
                : settingsStore.isDark ? 'border-slate-700 hover:border-slate-600' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'">
              <!-- Preview claro -->
              <div class="bg-slate-100 p-3 h-36 flex flex-col gap-2">
                <!-- Fake topbar -->
                <div class="bg-white dark:bg-slate-900 rounded-lg px-3 py-1.5 flex items-center gap-2 shadow-sm">
                ...
                <div class="flex gap-2 flex-1">
                  <div class="w-12 bg-white dark:bg-slate-900 rounded-lg shadow-sm"></div>
                  <div class="flex-1 flex flex-col gap-1.5">
                    <div class="bg-white dark:bg-slate-900 rounded-lg h-8 shadow-sm"></div>
                    <div class="bg-white dark:bg-slate-900 rounded-lg flex-1 shadow-sm"></div>
                  </div>
                </div>
              </div>
        ...
        <section class="rounded-2xl p-6 border transition-colors"
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between p-4 rounded-xl border transition-colors"
              :class="'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:bg-slate-800 dark:border-slate-700'">
              <div>
              ...
              <div @click="settingsStore.toggleNotifications()"
                ...
                <div class="absolute top-1 w-4 h-4 bg-white dark:bg-slate-900 rounded-full transition-all shadow-sm"
          ...
          <div class="space-y-4">
            <div v-for="field in passwordFields" :key="field.key" class="relative group">
              <label class="absolute -top-2.5 left-4 px-2 text-[9px] font-black uppercase tracking-widest z-10"
                :class="'bg-white dark:bg-slate-900 text-slate-400 dark:bg-slate-900 dark:text-slate-500'">
                {{ field.label }}
              </label>
              <div class="relative">
                <input
                  ...
                  :class="'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:border-blue-500 dark:bg-slate-800 dark:border-slate-700 dark:text-white dark:placeholder-slate-600 dark:focus:border-blue-500'"
            ...
            <button @click="cambiarPassword" :disabled="savingPassword"
              ...
              :class="'bg-blue-600 hover:bg-blue-700 text-white dark:bg-blue-600 dark:hover:bg-blue-500 dark:text-white'">
